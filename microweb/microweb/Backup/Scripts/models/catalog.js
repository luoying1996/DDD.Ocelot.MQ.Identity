...
window.CatalogView = Backbone.View.extend({
    ...
    IndexAnnouncement: function () {
        var agentId = $('#hagentId').val();
        $.ajax({
            //  url: "api/GetAnnouncementList/?AgentID=" + localStorage.AgentID,
            url: "api/GetAnnouncementList/?AgentID=" + agentId,
            dataType: "json",
            type: "GET",
            success: function (data) {
                ...
                if (data != null && data.Data != null && data.Data.length > 0) {
                    ...
                    localStorage.setItem("remarks", JSON.stringify(data));
    ...
    IndexAdFirst: function () {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: { W_Type: 1, BanerPostion: 0, ShopID: agentId },
            type: "GET",
            success: function (data) {
                if (data != null && data.Data != null && data.Data.length > 0) {
...
function showRemark(index) {
    //   localStorage.setItem("remarks", JSON.stringify(data));
    var remarkStr = localStorage.getItem('remarks');
    var remarkDa = JSON.parse(remarkStr);
    localStorage.setItem('remark', remarkDa.Data[index].Remark);
    window.open('/static/remark.html');
    //location.href = "/#page/remark";
}
