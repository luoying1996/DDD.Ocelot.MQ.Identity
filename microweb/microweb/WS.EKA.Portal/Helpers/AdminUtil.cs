    ...
    public class AdminUtil
    {
        ...
        public static string GetOpenID(string appid, string redirect_url, string code, string screct)
        {
            ...
            if (string.IsNullOrEmpty(code))
            {
                ...
                LogApp.Log4Net.Info("redirect_url：" + redirect_url);
                LogApp.Log4Net.Info("weixin：" + string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type=code&scope=snsapi_base&state={2}#wechat_redirect",
                  appid, redirect_url, new Random().Next(1000, 200000).ToString()));
                HttpContext.Current.Response.Redirect(string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type=code&scope=snsapi_userinfo&state={2}#wechat_redirect",
                  appid, redirect_url, new Random().Next(1000, 200000).ToString()));
            }
            else
            {
