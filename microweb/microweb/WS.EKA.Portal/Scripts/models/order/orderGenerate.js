...
function GoPay() {
    ...
    if (ShouldPayPrice > 0 && ordernamber > 0 && username != '') {
        if (paymenttype == "PreDeposits.aspx") {
		...
		else if (paymenttype.toLowerCase() == "weixin.aspx") {
			...
			var url = "http://fapi.6868xp.com/PayReturn/ZFPay/wxpay/pay.aspx?auth_code=" + auth_code + "&body=" + body + "&device_info=" + device_info + "&out_trade_no=" + out_trade_no + "&total_fee=" + total_fee + "&memid=" + memid + "&openid=" + openid;
