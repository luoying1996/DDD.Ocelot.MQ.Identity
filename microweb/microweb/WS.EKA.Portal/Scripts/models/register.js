...
window.RegisterView = Backbone.View.extend({
    ...
    GetCodeClick: function () {
        ...
        if (CheckPhone(phone)) {
            ...
            CheckPhoneExist(phone, function (res) { 
                if (res != null && res.return) {
                    alert('用户名已经存在！');
                    isExist = false;
                } else {
                    SendCodeToPhone(phone, function (res) { 
	...
	registerNextBtn: function () {
        ...
        if (pwd.length < 6) { alert('密码不少于6位！'); flag = false; return; }
    ...
    validation: function () {
		var pwd = $('#r_password').val(),
			...
			agree = true,
			flag = true;
			if (userName == "") { alert('用户名不能为空！'); flag = false; return; }
			if (!CheckPhoneExist(userName)) {
				return;
			}
		if (pwd == "") { alert('密码不能为空！'); flag = false; return; }
		if (pwd.length<6) { alert('密码不少于6位！'); flag = false; return; }
        ...
        $.ajax({
            url: 'api/account/userexist/' + commendPeople + "/" + localStorage.getItem("agentid"),
            dataType: 'json',
            async: false,
...
// 校验手机号是否已经注册
function CheckPhoneExist(phone,handle) {
    var isExist = false;
  
    $.ajax({
        url: 'api/account/userexist/' + phone + "/" + localStorage.getItem("agentid"),
        dataType: 'json',
        async: false,
        success: function (res) {
            console.log(res);
...
// 校验手机号是否已经注册
function CheckPhoneExists(phone, handle) {
    var isExist = false;
...
//向输入的手机号发送验证码
function SendCodeToPhone(phone,handle) {
    var isSend = false;
    $.ajax({
