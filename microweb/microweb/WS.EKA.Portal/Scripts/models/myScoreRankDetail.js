//积分对象
window.ScoreRankDetail = Backbone.Model.extend({
    default: {
    },
});
//积分对象集合
window.MyScoreRankDetailList = Backbone.Collection.extend({
    model: ScoreRankDetail,
    urlRoot: "api/getscorerankdetail/",
    url: function () {
...
//页面初始化
window.MyScoreRankDetailView = Backbone.View.extend({
    //页面对象
    el: "#jqt",
    //初始化对象
    initialize: function () {
        this.template = $('#ScoreRankDetailTemplate').html();
        this.collection = new MyScoreRankDetailList();
    ...
    render: function () {
        ...
        //加载数据
        var data = { title: "等级积分明细", loginUrl: localStorage.getItem('loginUrl'), mobile: localStorage.getItem('mobile') };
        //合并并加载页面
    ...
    fetchScoreDetailList: function () {
        ...
        this.collection.fetch({
            ...
            success: function () {
                ...
                var template;
                //加载空模板
                if (that.collection.length == 0 || MyScoreRankDetailModel.length == 0 || allCount == 0) {
                    template = $('#EmptyScorceRankDetialTemplate').html();
                    $("#MyScoreRankDetailList").html(Mustache.render(template, [], []));
                    $(".nav-page").html("");

                } else { //加载数据模板
                    $.each(myScoreDetails, function (index, d) {
                        if (d.OperateType == 0) {
                            d.LastOperateScore = '-' + d.LastOperateScore;
                        } else {
                            d.LastOperateScore = '+' + d.LastOperateScore;
                        }
                    });

                    template = $("#MyScoreRankDetailModel").html();
                    var data = { MyScoreRankDetailModel: myScoreDetails };
                    $("#MyScoreRankDetailList").html(Mustache.render(template, data, []));

                    //修改数据显示的效果
                    $("#MyScoreRankDetailList.dd").each(function () {
